...
CREATE
OR REPLACE FUNCTION generate_random_date(starting_timeframe INTEGER, ending_timeframe INTEGER) RETURNS DATE AS
$func$
DECLARE
starting_interval INTERVAL DEFAULT make_interval(years => starting_timeframe);
    ending_interval
INTERVAL DEFAULT make_interval(years => (ending_timeframe - starting_timeframe));
BEGIN
...
CREATE
OR REPLACE FUNCTION generate_random_date_arr(n INTEGER, starting_timeframe INTEGER,
