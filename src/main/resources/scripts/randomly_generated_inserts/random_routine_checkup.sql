...
CREATE
OR REPLACE PROCEDURE insert_random_medical_records()
    LANGUAGE plpgsql AS
$$
DECLARE
ruts TEXT ARRAY DEFAULT ARRAY(SELECT rut
                                  FROM elder);
    n
INTEGER DEFAULT cardinality(ruts);
BEGIN
FOR i in 1..n
        LOOP
...
CREATE
OR REPLACE PROCEDURE insert_random_routine_checkups()
    LANGUAGE plpgsql
AS
$$
DECLARE
elder_ruts        TEXT ARRAY DEFAULT array(SELECT rut
                                               FROM elder);
    admission_dates
DATE ARRAY DEFAULT ARRAY(SELECT admission_date
                                               FROM elder);
    m
INTEGER;
    n
INTEGER DEFAULT (SELECT count(*)
                                       FROM medical_record);
    calculated_height
DOUBLE PRECISION;
    calculated_weight
...
CASE;
            calculated_bmi
= (random() * (34.99 - 17.00) + 17.00);
            calculated_weight
= calculated_bmi * pow(calculated_height, 2);
SELECT(current_date - admission_dates[i]::date)
INTO m;
FOR j in 1..m
                LOOP
...
= (random() * 0.5);
                    IF
((is_negative) AND (calculated_bmi <= 17.00)) THEN
                        delta = abs(delta);
                    ELSIF
((NOT is_negative) AND (calculated_bmi >= 34.99)) THEN
                        delta = -delta;
                    ELSIF
(is_negative) THEN
                        delta = -delta;
END IF;
                    calculated_weight
= calculated_weight + delta;
                    calculated_bmi
= calculated_weight / pow(calculated_height, 2);
