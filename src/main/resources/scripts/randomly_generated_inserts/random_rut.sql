...
CREATE
OR REPLACE FUNCTION generate_rut(l INTEGER, u INTEGER) RETURNS TEXT AS
$func$
DECLARE
rut        TEXT;
    multiplier
...
(l < 1 OR u > 99) THEN
    ...
    rut
= floor(random() * (upper - lower + 1) + lower)::int::text;
...
CREATE
OR REPLACE FUNCTION generate_last_digit(rut TEXT) RETURNS CHARACTER AS
$func$
DECLARE
reverse     TEXT DEFAULT reverse(rut);
    n
INTEGER DEFAULT length(rut);
    digit
INTEGER;
...
CREATE
OR REPLACE FUNCTION generate_rut_arr(n INTEGER, l INTEGER, u INTEGER) RETURNS TEXT ARRAY AS
$func$
DECLARE
arr TEXT ARRAY;
BEGIN
